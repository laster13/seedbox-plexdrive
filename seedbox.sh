...
echo -e "${CCYAN}INSTALLATION${CEND}"
	...
	case $PORT_CHOICE in
		...
		2) 	## Mise en place des variables necéssaire au docker-compose
			...
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CGREEN}															  ${CEND}"
			echo -e "${CRED}		Pour que Unionfs fonctionne, il faut que l'arborescence montée en local corresponde			  ${CEND}"
			echo -e "${CRED}		exactement à celle montée par rclone (plexdrive crypté monté en clair par rclone).			  ${CEND}"
			echo -e "${CRED}		Il est donc nécéssaire de créer les mêmes dossiers que ceux existant sur Gdrive.			  ${CEND}"
			echo -e "${CGREEN}															  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			...
			FILMS=$(grep -E 'films|Films|FILMS|MOVIES|Movies|movies|VIDEOS|VIDEO|Video|Videos' ${VOLUMES_ROOT_PATH}/local.txt | cut -d: -f2 | cut -d ' ' -f2)
			SERIES=$(grep -E 'series|TV|tv|Series|Séries|SERIES|séries|Séries TV|SERIES TV|Series TV' ${VOLUMES_ROOT_PATH}/local.txt | cut -d: -f2 | cut -d ' ' -f2-3)
			ANIMES=$(grep -E 'ANIMES|Animes|Anime|Animation|animes' ${VOLUMES_ROOT_PATH}/local.txt | cut -d: -f2 | cut -d ' ' -f2)
			MUSIC=$(grep -E 'MUSIC|Music|music|Musiques|Musique|MUSIQUE|flac|Flac|MUSIQUES|musiques|musique' ${VOLUMES_ROOT_PATH}/local.txt | cut -d: -f2 | cut -d ' ' -f2)
		...
		3)
			...
			echo -e "${CGREEN}----------------------------${CEND}"
			echo ""
			echo -e "${CGREEN}   1) Tentative de récupération ${CCYAN}"rclone.conf"${CGREEN} sur le serveur ${CEND}"
			...
			case $PLEXDRIVE in
				1) ## création des fichiers plexdrive
					...
					then
						...
						echo ""
						echo -e "${CCYAN}Les fichiers de configuration ont été créés avec succés. Patienter 1mn le temps que le fichier cache.bolt soit constitué${CEND}"
						progress-bar 60
						sleep 60
						cd /mnt
		...
		5)
			...
			case $APPLI in
				...
				if mountpoint -q ${MPOINT}; then
					...
					echo ""
					echo -e "${CGREEN}IMPORTANT${CCYAN}: Les noms de bibliothèques doivent être identiques à ceux des dossiers créés pour Unionfs (sans accents) ${CEND}"
					progress-bar 20
					clear
					logo.sh
		...
		6)	
			...
			case $CONFIG in
				...
				F1=$(grep -E 'films|Films|FILMS|MOVIES|Movies|movies|VIDEOS|VIDEO|Video|Videos' categories.log | cut -d: -f1 | cut -d ' ' -f1)
				S1=$(grep -E 'series|TV|Series|Séries|SERIES|séries|Séries TV|SERIES TV|Series TV' categories.log | cut -d: -f1 | cut -d ' ' -f1)
				A1=$(grep -E 'ANIMES|Animes|Anime|Animation|animes' categories.log | cut -d: -f1 | cut -d ' ' -f1)
				M1=$(grep -E 'MUSIC|Music|music|Musiques|Musique|MUSIQUE|flac|Flac|MUSIQUES|musiques' categories.log | cut -d: -f1 | cut -d ' ' -f1)

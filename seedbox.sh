...
smallLoader() {
    echo ""
    echo ""
    echo -ne '[ + + +             ] 3s Merci de patienter \r'
    sleep 1
    echo -ne '[ + + + + + +       ] 2s Merci de patienter \r'
    sleep 1
    echo -ne '[ + + + + + + + + + ] 1s Merci de patienter \r'
    sleep 1
...
echo -e "${CCYAN}INSTALLATION ${CEND}"
	...
	echo -e "${CGREEN}   2) Configuration du docker-compose && Lancement Plexdrive && traefik ${CEND}"
	echo -e "${CGREEN}   3) Configuration rclone ${CEND}"
	echo -e "${CGREEN}   4) Configuration plexdrive v5.0.0 ${CEND}"
	...
	case $PORT_CHOICE in
		1) ## Installation de docker et docker-compose
			logo.sh
                        echo -e "${CCYAN}---------------------------------${CEND}"
			echo -e "${CRED}[  INSTALLATION DOCKER ET DOCKER-COMPOSE  ]${CEND}"
			echo -e "${CCYAN}---------------------------------${CEND}"
			echo ""                        
			apt-get install \
                        apt-transport-https \
		...
		2) 	## Mise en place des variables necéssaire au docker-compose
			...
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CCYAN}				LES VARIABLES CI DESSOUS SONT DEFINIES PAR DEFAULT DANS LES CONTAINERS			  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CRED}	${CCYAN}MountPoint:${CRED}	Montage crypté plexdrive monté en clair par défault dans ${CPURPLE}/mnt/rclone 	  ${CEND}"
			echo -e "${CRED}	${CCYAN}MountUnion:${CRED}	Montage Unionfs-fuse monté par défault dans ${CPURPLE}/mnt/Union 	  	  ${CEND}"
			echo -e "${CRED}	${CCYAN}MountLocal:${CRED}	Montage local par défault monté dans ${CPURPLE}/mnt/Pre 		  	  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CCYAN}				LA VARIABLE CI DESSOUS EST DEFINIE PAR DEFAULT SUR L HOTE			  	  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CCYAN}	${CGREEN}VOLUMES_ROOT_PATH:${CRED} Emplacement par défault des volumes sur l'hote ${CPURPLE}/mnt/docker		  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CGREEN}				VOUS POUVEZ LES MODIFIER OU NE RIEN SAISIR						  ${CEND}"
			...
			VAR=$(htpasswd -c /etc/apache2/.htpasswd $USERNAME)
			VAR=$(sed -e 's/\$/\$$/g' /etc/apache2/.htpasswd 2>/dev/null )
			export VAR
			echo "$VAR"

			## Création d'un fichier .env dans lequel viendront s'incrémenter les variables définies plus haut
			docker network create traefik_proxy 2>/dev/null
			docker network create torrent 2>/dev/null
			rm -rf /mnt/docker-compose.yml
			rm -rf /mnt/.env
			rm -rf ${VOLUMES_ROOT_PATH}/traefik/traefik.toml
			...
			services:
			  ...
			  sonarr:
			    ...
			    volumes:
			      ...
			      - ${VOLUMES_ROOT_PATH}/plexdrive/Union/TV:/tv
			    networks:
			      - proxy
			  ...
			  radarr:
			    ...
			    volumes:
			      ...
			      - ${VOLUMES_ROOT_PATH}/rutorrent/downloads:/downloads
			      - ${VOLUMES_ROOT_PATH}/plexdrive/Union/Movies:/movies
			    networks:
			      - proxy
			...
			echo -e "${CCYAN}----------------------------------------------------${CEND}"
			echo -e "${CRED}[  VERIFICATION DE LA CONFORMITE DU DOCKER-COMPOSE  ]${CEND}"
			echo -e "${CCYAN}----------------------------------------------------${CEND}"
			nano /mnt/docker-compose.yml
			smallLoader
			cd /mnt
			docker-compose up -d plexdrive traefik
			echo ""
			echo -e "${CCYAN}La configuration des variables et le lancement de plexdrive && traefik se sont bien déroulés ${CEND}"
			echo ""
			echo -e "${CGREEN}Le CLAIM de Plex n'est valable que 4mn d'ou la nécéssité de l'enregistrer rapidement ${CEND}"
			echo -e "${CGREEN}Dans l'éventualité où vous préférez lancer Emby, répondez non à la question ci dessous ${CEND}"
			echo ""
			read -rp "Utilisez vous Plex ? (o/n) : " EXCLUDE
				if [[ "$EXCLUDE" = "o" ]] || [[ "$EXCLUDE" = "O" ]]; then
    				cd /mnt
					docker-compose up -d plex && docker-compose rm -fs plex
					echo ""
			   		echo -e "${CCYAN}L'enregistrement du CLAIM est effectué, si vous souhaitez utiliser plex allez dans les appli choix 5 ${CEND}"
					smallLoader 
				else
			   		echo -e "${CCYAN}Vous pouvez lancer Emby en tapant le choix 5 "Lancement des applications" ${CEND}"
					smallLoader 
					seedbox.sh
				fi
			seedbox.sh
		...
		3)
			...
			logo.sh
			echo -e "${CCYAN}----------------------------${CEND}"
			echo -e "${CRED}[  CONFIGURATION DE RCLONE  ]${CEND}"
			echo -e "${CCYAN}----------------------------${CEND}"
			echo -e "${CCYAN}CONFIGURATION RCLONE ${CEND}"
			echo -e "${CGREEN}   1) Tentative de récupération rclone.conf sur le serveur ${CEND}"
			echo -e "${CGREEN}   2) Récupération rclone.conf à partir d'un autre serveur ${CEND}"
			...
			case $RCLONE in
				...
				echo -e "${CCYAN}Veuillez patienter, cela peut durer quelques secondes${CEND}"
					...
					then
						...
						echo -e "${CCYAN}Les fichiers de configuration sont déjà présents ${VOLUMES_ROOT_PATH}/plexdrive/.config/rclone .. Veuillez vérifier${CEND}"
						smallLoader
					fi
				seedbox.sh
				;;

				2) ## editer un fichier rclone.conf
				var=$(ls -a ${VOLUMES_ROOT_PATH}/plexdrive/config/.config/rclone/ 2>/dev/null | sed -e "/\.$/d" | wc -l)
				if [ "$var" -ne 0 ]
				then
					echo -e "${CCYAN}Les fichiers de configuration sont déjà présents ${VOLUMES_ROOT_PATH}/plexdrive/config/.config/rclone .. Veuillez vérifier${CEND}"
					smallLoader
					seedbox.sh
				fi
				read -rp "Voulez-vous copier un fichier de configuration ? (o/n) : " EXCLUDE
				...
				echo -e "${CCYAN}Les fichiers de configuration ont bien été créés${CEND}"
				smallLoader
				seedbox.sh
				;;
				...
				then
					...
					cp /root/.config/rclone ${VOLUMES_ROOT_PATH}/plexdrive/config/.config/rclone/
					seedbox.sh
				fi
				;;
		...
		4)
			export $(xargs </mnt/.env)
			echo -e "${CCYAN}---------------------------------${CEND}"
			echo -e "${CRED}[  CONFIGURATION DE PLEXDRIVE  ]${CEND}"
			echo -e "${CCYAN}---------------------------------${CEND}"
			echo -e "${CCYAN}CONFIGURATION Plexdrive V5.0.0 ${CEND}"
			...
			case $PLEXDRIVE in
				1) ## création des fichiers plexdrive
					...
					if [ "$RESULT1" -ne 0 ]
					then
						echo -e "${CCYAN}Les fichiers de configuration sont déjà présents ${VOLUMES_ROOT_PATH}/plexdrive/config/.config/.plexdrive .. Veuillez vérifier${CEND}"
						smallLoader
						seedbox.sh
					else
						echo -e "${CRED}--------------------------------------------------------------${CEND}"
						echo -e "${CCYAN}[  Une fois le token rentré, appuyer sur les touches CTRL-C  ]${CEND}"
						echo -e "${CCYAN}[  Nul besoin d'attendre la fin de création des fichiers     ]${CEND}"
						echo -e "${CRED}--------------------------------------------------------------${CEND}"
						trap "echo Vous avez appuyé sur les touches CTRL-C - Retour Menu" 2
						...
						echo -e "${CCYAN}Les fichiers de configuration ont été créés avec succés${CEND}"
						echo -e "${CCYAN}Patienter 1mn le temps que le fichier cache.bolt soit constitué  ${CEND}"
						sleep 60
						cd /mnt
						docker-compose restart plexdrive
						smallLoader
						seedbox.sh
				fi
				;;
		...
		6)	
			...
			docker exec -ti plex grep -E -o "PlexOnlineToken=.{0,22}" /config/Library/Application\ Support/Plex\ Media\ Server/Preferences.xml > ${VOLUMES_ROOT_PATH}/plexdrive/token.txt
			TOKEN=$(grep PlexOnlineToken ${VOLUMES_ROOT_PATH}/plexdrive/token.txt | cut -d '=' -f2 | cut -c2-21)
			export TOKEN
			...
			do
			echo -e "${CCYAN}------------------${CEND}"
			echo -e "${CRED}[  CONFIGURATION  ]${CEND}"
			echo -e "${CCYAN}------------------${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------${CEND}"
			echo -e "${CRED}[  CONFIGURATION PLEX_AUTOSCAN - UNIONFS_CLEANER - PLEX_DUPEFINDER ]${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------${CEND}"
			echo -e "${CGREEN}   1) plex_autoscan (Uniquement pour Plex) ${CEND}"
			echo -e "${CGREEN}   2) unionfs_cleaner (Plex et Embi) ${CEND}"
			echo -e "${CGREEN}   3) plex_dupefinder (Uniquement pour Plex) ${CEND}"
			echo -e "${CGREEN}   4) Retour Menu Principal ${CEND}"
			read -p "Config choix [1-4]: " -e -i 1 CONFIG
			case $CONFIG in
				1)
				rm -rf ${VOLUMES_ROOT_PATH}/plex/config/plex_autoscan/config.json
				touch ${VOLUMES_ROOT_PATH}/plexdrive/Union/mounted.bin
				...
				echo -e "${CCYAN}------------------------------------------------------------------${CEND}"
				echo -e "${CRED}[  VERIFICATION DE LA CONFORMITE DU CONFIG.JSON DE PLEX_AUTOSCAN  ]${CEND}"
				echo -e "${CCYAN}------------------------------------------------------------------${CEND}"
				smallLoader
				...
				2)
				rm -rf ${VOLUMES_ROOT_PATH}/plexdrive/config/unionfs_cleaner/config.json
				...
				EOF
				echo -e "${CCYAN}--------------------------------------------------------------------${CEND}"
				echo -e "${CRED}[  VERIFICATION DE LA CONFORMITE DU CONFIG.JSON DE UNIONFS_CLEANER  ]${CEND}"
				echo -e "${CCYAN}--------------------------------------------------------------------${CEND}"
				smallLoader
			...
			EOF
			echo -e "${CCYAN}--------------------------------------------------------------------${CEND}"
			echo -e "${CRED}[  VERIFICATION DE LA CONFORMITE DU CONFIG.JSON DE PLEX_DUPEFINDER  ]${CEND}"
			echo -e "${CCYAN}--------------------------------------------------------------------${CEND}"
			smallLoader
			nano ${VOLUMES_ROOT_PATH}/plex/config/plex_dupefinder/config.json
			echo -e "${CCYAN}La configuration de plex_dufinder s'est déroulée avec succés${CEND}"
			smallLoader
			;;
		...
		7)	
			...
			do
			echo -e "${CCYAN}-------------------------------------------------------------------${CEND}"
			echo -e "${CRED}  			SAUVEGARDE DES VOLUMES			    ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------${CEND}"
			...
			echo -e "${CPURPLE}	Les points de montage ainsi que les librairies Plex et Emby ne doivent pas être concernés par la sauvegarde	  ${CEND}"
			echo -e "${CPURPLE}	Vous devez les mentionner dans les exclusions.									  ${CEND}"
			echo -e "${CGREEN}															  ${CEND}"
			echo -e "${CGREEN}															  ${CEND}"
			echo -e "${CRED}-------------------------------------------------------------------------------------------------------------------------${CEND}"

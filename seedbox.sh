...
echo -e "${CCYAN}INSTALLATION${CEND}"
	...
	case $PORT_CHOICE in
		...
		2) 	## Mise en place des variables necéssaire au docker-compose
			...
			echo -e "${CRED}	${CCYAN}WATCHER_FQDN:${CRED}		watcher.${DOMAIN}							  ${CEND}"
		...
		6)	
			...
			case $CONFIG in
				...
				ID_SERIE=$(grep -E 'series|TV|tv|Series|SERIES|SERIES TV|Series TV|series tv|serie tv|serie TV|series TV|Shows' categories.log | cut -d: -f1 | cut -d ' ' -f1)
